...
/***/ (function(module, exports) {
	...
	function SamlDao(RESOURCE, ArtifactoryDaoFactory) {
	  return ArtifactoryDaoFactory().setPath(RESOURCE.SAML_CONFIG);
	}
...
/***/ (function(module, exports) {
	...
	var TreeBrowserDao = exports.TreeBrowserDao = (function () {
	    function TreeBrowserDao(RESOURCE, ArtifactoryHttpClient, $q, $injector, ArtifactoryStorage) {
	    ...
	    _createClass(TreeBrowserDao, {
	        ...
	        _loadChildren: {
	            value: function _loadChildren(node, parent) {
	                var _this = this;
	                ...
	                return this.artifactoryHttpClient.post(this.RESOURCE.TREE_BROWSER + "?compacted=" + compact + "&$no_spinner=true", node).then(function (result) {
	                    return _this._transformData(result.data, parent);
	                });
	            }
	        },
	        _transformData: {
...
/***/ (function(module, exports) {
	...
	function TreeNodeFactory($q, $injector, RESOURCE, ArtifactoryHttpClient, ArtifactWatchesDao, ArtifactActionsDao, NativeBrowser, ArtifactXrayDao) {
	    return function (data) {
	        return new TreeNode(data, $q, $injector, RESOURCE, ArtifactoryHttpClient, ArtifactWatchesDao, ArtifactActionsDao, NativeBrowser, ArtifactXrayDao);
	    };
	}
	
	var TreeNode = (function () {
	    function TreeNode(data, $q, $injector, RESOURCE, ArtifactoryHttpClient, ArtifactWatchesDao, ArtifactActionsDao, NativeBrowser, ArtifactXrayDao) {
	    ...
	    _createClass(TreeNode, {
	        updateFavoriteState: {
	            value: function updateFavoriteState() {
	                var isFav = false;
	                if (!this.isRepo()) isFav = false;else if (localStorage.favoritesRepos) {
	        ...
	        findNodeByPath: {
	            value: function findNodeByPath(path, startIndex) {
	                ...
	                return this.getChildren().then(function (children) {
	                    ...
	                    if (child) return child.findNodeByPath(path, startIndex, includeArchives);else return _this;
	                })["catch"](function () {
	                    return _this;
	                });
	            }
	            ...
	            value: function load() {
	                ...
	                if (this.tabs || this.actions) {
	                    ...
	                    return this.artifactoryHttpClient.post(this.RESOURCE.TREE_BROWSER + "/tabsAndActions", data).then(function (response) {
	                        ...
	                        if (removeIndex !== -1) _this.actions.splice(removeIndex, 1);
	
	                        var index = _this.actions.indexOf(_.findWhere(_this.actions, { name: "Watch" }));
	                        if (index === -1) index = _this.actions.indexOf(_.findWhere(_this.actions, { name: "Unwatch" }));
	                        if (index === -1) index = _this.actions.indexOf(_.findWhere(_this.actions, { name: "Move" }));
	                        if (index === -1) index = _this.actions.indexOf(_.findWhere(_this.actions, { name: "Refresh" }));
	                        if (_this.nativeBrowser.isAllowed(_this)) _this.actions.splice(index + 1, 0, {
	            ...
	            value: function getDownloadPath() {
	                ...
	                if (!this.downloadPathPromise) {
	                    ...
	                    this.downloadPathPromise = this.artifactActionsDao.perform({ action: "download" }, data).$promise.then(function (response) {
	                        ...
	                        if (response.data.xrayValidation) _this.xrayShouldValidate = response.data.xrayValidation;
	                    });
	                }
	                ...
	                if (!this.isInsideArchive() && this.isLocal() && !this.isTrashcan() && !this.isInTrashcan()) {
	                    return this.artifactWatchesDao.status({ repoKey: this.repoKey, path: this.path }).$promise.then(function (action) {
	                        ...
	                        if (action && action.name) {
	                            ...
	                            else _this.actions.push(action);
	                        } else {
	                            // Remove the previous action if there is no new action
	                            if (previousAction) _.remove(_this.actions, previousAction);
	                        }
	                        return _this;
	                    });
	                } else {
...
/***/ (function(module, exports) {
	...
	function SupportPageDao(RESOURCE, ArtifactoryDaoFactory) {
	    return ArtifactoryDaoFactory().setPath(RESOURCE.SUPPORT_PAGE + "/:action/:filename").setCustomActions({
	        generateBundle: {
	            method: "POST",
	            isArray: true,
...
//# sourceMappingURL=artifactory_dao.js.1354.map